...
(* variable and parameter definitions hashtable *)
let named_values : (string, Llvm.llvalue * Llvm.lltype * Ast.pass_by) Hashtbl.t
    =
  Hashtbl.create 10

(* function definitions hashtable *)
let function_names : (string, Llvm.lltype * Llvm.lltype list) Hashtbl.t =
  Hashtbl.create 10
...
(* function to get corresponding llvm type from grace's variable type *)
let rec var_type_to_lltype = function
  | Ast.Int -> i32_t
  | Ast.Char -> i8_t
  | Ast.Array (data_type, s) ->
      Llvm.array_type (var_type_to_lltype data_type) (Option.get s)
  | Ast.Nothing -> void_t
...
let codegen_var_def (var_def_n : Ast.var_def Ast.node) =
  match var_def_n.node with
  | id, data_type ->
      let the_type = var_type_to_lltype data_type in
...
let codegen_param_def (param_def_node : Ast.param_def Ast.node) =
  match param_def_node.node with
  | id, data_type, pass_by -> (
      match pass_by with
      | Ast.Value ->
          let the_type = var_type_to_lltype data_type in
...
let rec flatten_l_val (l_val_n : Ast.l_value Ast.node) =
  match l_val_n.node with
  | Ast.ArrayAccess (l_val_n, e_n) ->
      let l_val_info, e_lst = flatten_l_val l_val_n in
      let e = codegen_expr e_n in
      (l_val_info, e :: e_lst)
  | _ -> (codegen_l_value l_val_n, [])

and codegen_l_value (l_val_n : Ast.l_value Ast.node) =
  match l_val_n.node with
  | Ast.Id id -> (
  ...
  | Ast.ArrayAccess _ ->
      ...
      let the_elem_ptr =
        Llvm.build_gep the_type the_var_ptr (Array.of_list e_lst)
          "array_elem_tmp" builder
      in
      (the_elem_ptr, unroll_lltype the_type n)

and codegen_expr (expr_n : Ast.expr Ast.node) =
  match expr_n.node with
  | Ast.LitInt i -> c32 i
...
and codegen_func_call (func_call_n : Ast.func_call Ast.node) =
  match func_call_n.node with
  | id, e_n_l ->
      let func_decl =
...
let rec codegen_cond (cond_n : Ast.cond Ast.node) =
  match cond_n.node with
  | Ast.UnLogicOp (ulo, cond_n) -> (
      match ulo with
      | Ast.Not -> Llvm.build_not (codegen_cond cond_n) "not_tmp" builder)
  | Ast.BinLogicOp (cond_1_n, blo, cond_2_n) ->
      ...
      Llvm.build_phi
        [ (lhs, the_block); (rhs, the_cond) ]
        "bin_cond_tmp" builder
...
let rec codegen_block (block_n : Ast.block Ast.node) =
  match block_n.node with
  | stmt_n_l -> List.iter (fun stmt_n -> codegen_stmt stmt_n) stmt_n_l

and codegen_stmt (stmt_n : Ast.stmt Ast.node) =
  match stmt_n.node with
  | Ast.Empty -> ()
  ...
  | Ast.If (cond_n, stmt_1_n_o, stmt_2_n_o) ->
      ...
      if Option.is_some stmt_2_n_o then codegen_stmt (Option.get stmt_2_n_o);
      Llvm.build_br the_endif builder |> ignore;
...
let codegen_func_decl (func_decl_n : Ast.func_decl Ast.node) =
  match func_decl_n.node with Ast.{ node = header; _ } -> ()
