...
*)
  ...
  let rec gen_block frame caller_path (block : Ast.block)
      (final_block : Llvm.llbasicblock)
      (ret_list : (Llvm.llvalue * Llvm.llbasicblock) list) =
    let rec aux stmts ret_list =
      match stmts with
      | [] -> ret_list
      | hd :: tl -> (
          let ret_val_list =
            gen_stmt frame caller_path hd final_block ret_list
          in
          match Llvm.block_terminator (Llvm.insertion_block builder) with
          | None -> aux tl ret_val_list
          | Some inst -> (
              match Llvm.instr_opcode inst with
              | Llvm.Opcode.Ret -> aux [] ret_val_list
              | _ -> aux tl ret_val_list))
    in
    aux block.stmts ret_list
  and gen_stmt frame caller_path (stmt : Ast.stmt)
      (final_block : Llvm.llbasicblock)
      (ret_list : (Llvm.llvalue * Llvm.llbasicblock) list) =
    match stmt with
    | Ast.Empty _ ->
        Llvm.build_is_null (c32 0) "empty" builder |> ignore;
        ret_list
    | Ast.Assign (l_val, expr) ->
        ...
        ret_list
    | Ast.Block block -> gen_block frame caller_path block final_block ret_list
    ...
    | Ast.If (cond, stmt1_o, stmt2_o) ->
        ...
        let then_ret_list =
          gen_stmt frame caller_path (Option.get stmt1_o) final_block ret_list
        in
        ...
        let else_ret_list =
          if Option.is_some stmt2_o then
            gen_stmt frame caller_path (Option.get stmt2_o) final_block
              then_ret_list
          else then_ret_list
        in
    ...
    | Ast.While (cond, stmt) ->
        ...
        let body_ret_list =
          gen_stmt frame caller_path stmt final_block ret_list
        in
        (match Llvm.block_terminator (Llvm.insertion_block builder) with
        ...
        | Some expr -> (
            ...
            | Ast.EFuncCall Ast.{ ret_type; _ } ->
                if ret_type = Ast.Nothing then (
                  ...
                  ignore (Llvm.build_br final_block builder);
                  (ret_val, Llvm.insertion_block builder) :: ret_list)
            | _ ->
                ignore (Llvm.build_br final_block builder);
                (ret_val, Llvm.insertion_block builder) :: ret_list))
  in
  ...
  let rec gen_func_def (func : Ast.func) =
    ...
    let final_block = Llvm.append_block context "final" func_decl in
    ...
    let ret_list =
      gen_block frame
        ...
        (Option.get func.body) final_block []
    in
    let ret_list = List.rev ret_list in
    let current_block = Llvm.insertion_block builder in
    match (ret_list, Llvm.block_terminator current_block) with
    | [], None ->
        Llvm.delete_block final_block;
        ignore (Llvm.build_ret_void builder)
    | [], Some _ -> Llvm.delete_block final_block
    | ret_list, None ->
        Llvm.delete_block current_block;
        Llvm.position_at_end final_block builder;
        let ret_phi =
          Llvm.build_empty_phi
            (lltype_of_scalar func.ret_type)
            "ret_phi" builder
        in
        List.iter
          (fun (ret_val, ret_block) ->
            Llvm.add_incoming (ret_val, ret_block) ret_phi)
          ret_list;
        ignore (Llvm.build_ret ret_phi builder)
    | ret_list, Some _ ->
        Llvm.position_at_end final_block builder;
        let ret_phi =
          Llvm.build_empty_phi
