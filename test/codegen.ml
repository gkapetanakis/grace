...
let () =
  ...
  let typ = Codegen.var_type_lltype (Ast.Array(Ast.Char, Some 6)) in
  let xxx = Llvm.build_gep typ xx [|Codegen.c32 0; Codegen.c32 3|] "gep" Codegen.builder in
  let y = Llvm.build_load (Codegen.i8_t) xxx "load" Codegen.builder in
  Llvm.build_ret y Codegen.builder |> ignore;
  (*Llvm.dump_module Codegen.the_module;*)
  Llvm.dump_value y; print_newline ();
  Llvm_analysis.assert_valid_function f;
  Llvm.dump_type (Llvm.type_of xxx); print_newline ();
